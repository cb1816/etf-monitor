...
// Scarica lo screener Morningstar Italia (universo ETF Borsa Italiana ETEXG$XMIL)
// e restituisce l'oggetto DATA usato dal frontend.
...
const MACROS = ['Azionari', 'Obbligazionari', 'Bilanciati', 'Flessibili', 'Monetari', 'Alternativi', 'Materie Prime', 'Altro'];

function macroOf(cat) {
  if (!cat) return null;
  for (const m of MACROS) if (cat.startsWith(m)) return m;
  if (/^Materie/i.test(cat)) return 'Materie Prime';
  if (/cripto|digital/i.test(cat)) return 'Alternativi';
...
async function fetchScreener() {
  ...
  for (let page = 1; page <= 2; page++) {
    const url = `${API}?page=${page}&pageSize=5000&sortOrder=Name%20asc&outputType=json&version=1&languageId=it-IT&currencyId=EUR&universeIds=ETEXG%24XMIL&securityDataPoints=${encodeURIComponent(DATAPOINTS)}`;
